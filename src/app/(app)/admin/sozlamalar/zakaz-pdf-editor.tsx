...
import { zakazPdfSaqlaAction, zakazPdfTestAction } from "./actions";

export function ZakazPdfEditor({
  ...
  const onSave = () =>
    startSave(async () => {
      const res = await zakazPdfSaqlaAction({ token, chatId, topicId, autoEnabled });
      if (res.ok) {
  ...
  const onTest = () =>
    startSend(async () => {
      const res = await zakazPdfTestAction();
      if (res.ok) toast.success(`Sinov yuborildi — zakaz #${res.orderId}.`);
      else toast.error(res.error);
    });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Zakaz <b>&quot;Zakaz qabul qilindi&quot;</b> holatiga o&apos;tganda nakladnoy <b>PDF</b> sifatida shu guruh
        topigiga yuboriladi. Sahifadagi &quot;Telegramga yuborish&quot; tugmasi orqali qo&apos;lda ham yuborsa bo&apos;ladi.
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        />
        Zakaz qabul qilinganda avtomatik yuborilsin
      </label>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button onClick={onTest} disabled={sending} variant="secondary" className="h-10 rounded-xl">
          {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="mr-1.5 h-4 w-4" /> Sinov yuborish</>}
        </Button>
      </div>
      <p className="text-[11px] text-muted-foreground">
        &quot;Sinov yuborish&quot; — eng oxirgi qabul qilingan zakazni joriy sozlama bilan darhol yuboradi (config tekshiruvi uchun).
      </p>
    </div>
  );
}
