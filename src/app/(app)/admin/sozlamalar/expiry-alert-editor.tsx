...
import { expiryAlertSaqlaAction, expiryAlertYuborAction } from "./actions";

export function ExpiryAlertEditor({
  ...
  const onSave = () =>
    startSave(async () => {
      const res = await expiryAlertSaqlaAction({ token, chatId, topicId, autoEnabled });
      if (res.ok) {
  ...
  const onSend = () =>
    startSend(async () => {
      const res = await expiryAlertYuborAction();
      if (res.ok) {
        toast.success(res.count > 0
          ? `Yuborildi — ${res.count} ta shoshilinch partiya.`
          : "Yuborildi — muddati o'tgan yoki kritik partiya topilmadi.");
      } else toast.error(res.error);
    });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        ...
        Muddati o&apos;tgan va kritik (≤3 kun) partiyalar shu guruh topigiga yuboriladi.
        14 kunlik ogohlantirishlar xabarga KIRMAYDI — faqat soni ko&apos;rsatiladi, aks holda
        muhim qatorlar ko&apos;milib ketardi. Bot guruhga a&apos;zo va topikka
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        />
        Har kuni avtomatik yuborilsin (10:30, Toshkent · shoshilinch partiya bo'lsa)
      </label>
      ...
      <p className="text-[11px] text-muted-foreground">
        &quot;Hozir yuborish&quot; — joriy sozlama bilan sinov uchun darhol yuboradi (shoshilinch partiya bo&apos;lmasa ham xabar beradi).
      </p>
    </div>
  );
}
