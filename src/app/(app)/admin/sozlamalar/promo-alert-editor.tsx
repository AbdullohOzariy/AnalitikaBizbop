...
import { promoAlertSaqlaAction, promoAlertYuborAction } from "./actions";

export function PromoAlertEditor({
  ...
  const onSave = () =>
    startSave(async () => {
      const res = await promoAlertSaqlaAction({ token, chatId, topicId, autoEnabled });
      if (res.ok) {
  ...
  const onSend = () =>
    startSend(async () => {
      const res = await promoAlertYuborAction();
      if (res.ok) {
        toast.success(res.count > 0
          ? `Yuborildi — ${res.count} ta aksiya.`
          : "Yuborildi — bugun boshlanadigan/tugaydigan aksiya yo'q.");
      } else toast.error(res.error);
    });
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        ...
        Bugun BOSHLANADIGAN (narx yorlig&apos;ini almashtirish) va bugun TUGAYDIGAN
        (ertaga asl narxga qaytarish) aksiyalar SKU narxlari bilan yuboriladi. Davom
        etayotgan aksiyalar takror yuborilMAYDI. Bot guruhga a&apos;zo va topikka
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        />
        Har kuni avtomatik yuborilsin (08:00, Toshkent · bugun aksiya bo'lsa)
      </label>
      ...
      <p className="text-[11px] text-muted-foreground">
        &quot;Hozir yuborish&quot; — joriy sozlama bilan sinov uchun darhol yuboradi (aksiya bo&apos;lmasa ham xabar beradi).
      </p>
    </div>
  );
}
