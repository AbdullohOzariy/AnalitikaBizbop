...
import { Loader2, Save, CheckCircle2, Trash2, ArrowRight, FileDown } from "lucide-react";
import { formatDateTimeUZ } from "@/lib/format";
import { cn } from "@/lib/utils";
import { updateTransferItemsAction, confirmTransferAction, deleteTransferAction } from "../../actions";

export type DetailData = {
  id: number; status: string; targetDays: number; note: string;
  fromBranch: string; toBranch: string; createdBy: string; createdAt: string; confirmedAt: string | null;
  items: { productId: number; code: number; name: string; sub: string | null; qty: number }[];
};
...
export function KochirishDetail({ data }: { data: DetailData }) {
  ...
  const save = () => {
    ...
    startSave(async () => {
      const res = await updateTransferItemsAction(data.id, chosen, note);
      if (res.ok) { toast.success("Saqlandi."); router.refresh(); } else toast.error(res.error);
    });
  };
  const confirm = () => {
    if (!window.confirm("Tasdiqlash — hujjat qulflanadi (tahrirlash yopiladi). Davom etilsinmi?")) return;
    startAct(async () => {
      const res = await confirmTransferAction(data.id);
      if (res.ok) { toast.success("Tasdiqlandi."); router.refresh(); } else toast.error(res.error);
    });
  };
  const remove = () => {
    if (!window.confirm(`#${data.id} ko'chirish o'chirilsinmi?`)) return;
    startAct(async () => {
      const res = await deleteTransferAction(data.id);
      if (res.ok) { toast.success("O'chirildi."); router.push("/logistika?tab=kochirish"); } else toast.error(res.error);
    });
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card px-4 py-3">
        <span className="inline-flex items-center gap-1.5 text-sm font-medium">
          {data.fromBranch} <ArrowRight className="h-4 w-4 text-muted-foreground" /> {data.toBranch}
        </span>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-1.5">
          <a href={`/api/kochirish/${data.id}/pdf`} target="_blank" rel="noopener noreferrer"
            title="Ko'chirish ro'yxati (PDF)"
            ...
            <FileDown className="h-3.5 w-3.5" /> Ro&apos;yxat (PDF)
          </a>
          {isDraft ? (
            <>
      ...
      <div className="space-y-1">
        ...
        <Input value={note} disabled={!isDraft || busy} onChange={(e) => setNote(e.target.value)} placeholder="Ko'chirish haqida izoh..." className="h-9" />
      </div>

      <p className={cn("text-[11px] text-muted-foreground", isDraft ? "" : "hidden")}>
        ⚠ Filial qoldig&apos;i kunlik importdan keladi — tasdiqlash hujjatni qulflaydi, qoldiq avtomatik ayirilmaydi.
        Fizik ko&apos;chirish bajarilgach, qoldiqlar keyingi import bilan yangilanadi.
      </p>
    </div>
  );
}
