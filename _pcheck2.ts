...
(async () => {
  ...
  if (res.version !== "v3") { fs.writeFileSync("_pcheck.result", "version=" + res.version); return; }
  ...
  const out = "OK rows=" + res.productRows.length + " groups=" + res.categoryRowCount + " skus=" + skus.size + " noCat=" + noCat + " branchSums=" + JSON.stringify(Object.fromEntries(Object.entries(byBranch).map(([k, v]) => [k, Math.round(v)])));
  fs.writeFileSync("_pcheck.result", out);
})().catch((e) => fs.writeFileSync("_pcheck.result", "FAIL: " + e.message)).finally(() => prisma.$disconnect());
