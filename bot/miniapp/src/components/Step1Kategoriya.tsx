...
interface Props {
  ...
  /** Pastda qoralama banneri turibdimi — plitkalarni bosib qolmasligi uchun joy ochamiz */
  bannerJoyi?: boolean
}
...
export default function Step1Kategoriya({ onTanla, bannerJoyi = false }: Props) {
  ...
  return (
    // Hujjatda scrollport yo'q (html/body/#root height:100%), shuning uchun scroll
    // ichkarida: kichik ekran / compact rejimda pastki 2 plitkaga yetib bo'lmasdi.
    // Konvensiya Step2/Step3 dagidek — h-full + ichki flex-1 overflow-y-auto.
    <div className="flex h-full flex-col bg-tg-bg">
      <div className="flex flex-1 flex-col overflow-y-auto px-4 pb-6 pt-7">
        {/* Brend bar */}
        <motion.div
        ...
        {/* Qoralama banneri fixed va ~145px joy egallaydi — oxirgi qator plitkalar uning
            ostida qolmasligi uchun scroll oxiriga bo'sh joy qo'yamiz (padding-bottom
            flex-ustun scroll konteynerlarida ishonchsiz, shuning uchun element). */}
        {bannerJoyi && <div aria-hidden className="h-40 shrink-0" />}
      </div>
    </div>
  )
}
